...
- REQ-1: Each documentation page is one Markdown file under `content/<major>/` (`content/v1/` for 1.x, `content/v0/` for 0.23) and is rendered at `/docs/<major>/<section>/<slug>` for every version, the latest included; `/docs/<major>` redirects to the version's first page in navigation order (user review 2026-09-25: the index page was removed); `/docs`, `/docs/<section>/<slug>` and any `/docs/...` path without a version segment redirect to the same path under the current major. Source: brief decisions 5, 6, 7; user answer 2026-09-23 (final URL scheme).
...
- REQ-6: When the reader switches version on a page whose slug exists in the target version, the same slug opens in that version; otherwise the target version's first page opens with a notice naming the page and the version in which it does not exist. Source: brief decision 7.
...
- EDGE-1: reader switches to v0 on a page that exists only in v1 → the v0 first page opens with the notice "This page does not exist in 0.23" (REQ-6).
