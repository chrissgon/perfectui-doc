...
- Measurement of `@chrissgon/perfectui` 1.0.0-beta.1 on 2026-09-24 with `gzip -9 -n -c <file> | wc -c` (no file name in the header, the competitors' method, ADR-0007; corrected from the 2026-09-23 measurement of 1.0.0-beta.0 with `gzip -9 -c <file>`, which added the file name): `dist/perfectui.css` 3,221 B (raw 15,719 B), `dist/css/core.css` 868 B, `dist/js/index.js` 493 B, `dist/js/mode.js` 311 B; largest component stylesheet `form.css` 1,156 B; five fallback files under `dist/js/fallbacks/` (anchor-positioning, checkbox-indeterminate, command-for, dialog-closedby, interest-for)
...
- PROOF-7: Every rule lives in a cascade layer, so any plain CSS wins without `!important`; with Tailwind v4 one line declaring the layer order (`@layer theme, base, pui, components, utilities;`) makes every utility win and keeps Preflight below the library; Tailwind's dark variant can share `data-pui-mode`. Evidence: docs/tailwindcss.md of the library, rewritten 2026-09-24 after measuring Tailwind 4.3.3 and 3.4.17 against 1.0.0-beta.1 (without the declaration the import order either ignores utilities or lets Preflight remove button fills). Source: library docs/tailwindcss.md; measurement 2026-09-24.
...
- PROOF-11: 1.0 is a rewrite of every 0.x release (0.23 is the last) and a written migration guide covers every change from 0.23 in eight sections. Evidence: MIGRATION.md ("`1.0.0` is a rewrite", sections 1 to 8); PRD F-4. Source: MIGRATION.md; PRD F-4.
