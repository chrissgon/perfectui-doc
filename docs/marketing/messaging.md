...
- OPEN-1: Name the compared libraries in the size bars (Pico, Beer CSS, Bootstrap, UIkit, Bulma) with their versions and the measurement date? Blocks: SECTION-2's demo. Recommended: yes; the research measured them with the same method on the same day, and a comparison without names is the kind of claim the reader distrusts.
- OPEN-2: Hero headline: "The bare minimum for elegant interfaces" (the README's own words) or "Three classes, no framework, 3.2 kB" (the proof as the headline)? Blocks: SECTION-1's headline. Recommended: the first as headline and the second as the line under it, because the first names the idea and the second proves it.
- OPEN-3: A "used by" or sponsors section, as tailwindcss.com has. Blocks: nothing. Recommended: none at launch; there are no logos or numbers to show (213 weekly downloads counts machines), and an empty social-proof section costs more than its absence.

## Readiness

- Ready for design-ui: yes for the section list and demos; OPEN-2 decides the hero's two lines and can be answered during the landing review.
