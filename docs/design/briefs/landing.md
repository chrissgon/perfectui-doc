...
- SECTION-6 With Tailwind, not instead of it. Headline: Your utilities always win. Body: One `@layer` line keeps Tailwind's reset below perfectui and every utility above it. Demo: the code line `class="pui-btn pui-solid pui-theme w-full"` with the button stretching to full width as `w-full` is added; a second line showing the dark variant pointed at `data-pui-mode`. Call to action: "Tailwind guide".
